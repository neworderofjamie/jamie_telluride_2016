...
def add_connection(start_x, start_y,
    ...
    if end_cost != 0xFF:
        # Convert average cost of the two vertices to delay
        delay = 1.0 + ((float(start_cost) + float(end_cost)) * 0.5 * (7.0 / 256.0))
...
# What weight should we use to cause spikes
...
sim.Projection(stim, neurons,
               sim.FromListConnector([(0, get_neuron_index(stim_x, stim_y, cost_image.shape[0]), weight, 1.0)]),
               sim.StaticSynapse())
...
# Create image to display path information
path_image = np.zeros((cost_image.shape[0], cost_image.shape[1], 3))

# Add pixels indicating stim and end to image
path_image[stim_x, stim_y] = (0.0, 1.0, 0.0)
path_image[end_x,  end_y] = (0.0, 0.0, 1.0)
